...
5. **Financements pour l'entreprise.** Renseigne le bloc **`financements`** : les **appels à projets** (compétitifs, datés) et **subventions** (guichets ouverts) auxquels **CPTS Pilot, l'entreprise**, peut candidater — national (Bpifrance / France 2030 santé numérique, JEI, CII…) + régional (France 2030 Nouvelle-Aquitaine, Guide des aides `les-aides.nouvelle-aquitaine.fr`). Distingue bien `appels_projets` vs `subventions` et marque `perimetre` (`national`/`regional`). C'est pour financer l'entreprise, pas les CPTS clientes.
6. **Sépare le national du local.** Tout ce qui concerne le 64 / Nouvelle-Aquitaine (appels à projets ARS Nouvelle-Aquitaine, CPTS du 64, concurrents et signaux locaux) va dans le bloc **`local`** (jamais dans les sections nationales), qui reprend la même structure qu'un rapport.
7. **AJOUTE** (sans jamais écraser) un nouvel objet à la fin du tableau `journal` de `./agents/orion.json`, au format exact décrit dans ta fiche (champs : `updated_at`, `trigger`: `"ronde auto"`, `synthese`, `reglementaire`, `concurrence`, `opportunites`, `signaux`, `predictions`, `coups_avance`, `sources`, `financements`, `local`, `note`).
